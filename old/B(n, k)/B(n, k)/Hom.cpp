...
//Testing: Done.
basis e(vector<Perm> YS, vector<int> signs, int l, chain c) {
	basis Answer;
	for(int i = 0; i < l; i++) {
		const chain temp = c * (YS[i]);
		basis tempbas;
		tempbas = temp;
		tempbas *= signs[i];
		Answer += tempbas;
...
//Tested
basis e(vector<Perm> YS, vector<int> signs, int l, basis b) {
	basis Answer;
	sum stuff = b.Insides();
	sum::const_iterator A;
	for(A = stuff.begin(); A != stuff.end(); ++A) {
...
vector<double> project(basis b, vector<basis> B) {
	...
	for(vector<basis>::iterator iter = B.begin(); iter != B.end(); ++iter) {
		...
		for(list<chain>::const_iterator iter2 = TempSup.begin(); iter2 != TempSup.end(); ++iter2) {
			bool found = false;
			for(vector<chain>::iterator iter3 = SupportVec.begin(); iter3 != SupportVec.end(); ++iter3) {
				if (*iter3 == *iter2) {
					found = true;
					break;
				}
